...
The `magrittr` pipe operator (`%>%`) is a tool for chaining together a sequence of R functions.[^02-tidyverse-3] To demonstrate, consider the following commands that sort a data frame and then retain the first 10 rows:

[^02-tidyverse-3]: In R 4.1.0, a native pipe operator `|>` was introduced as well. In this book, we use the `magrittr` pipe since users on older versions of R will not have the new native pipe.
